    ...
    for (const pinEmitter of pinsInNetwork) {
      ...
      for (const pinTarget of pinsInNetwork) {
        ...
        if (emitterDir === "x+" || emitterDir === "x-") {
          ...
          const withinActivateDistance =
            Math.abs(deltaYOrthogonal) <= activateDistance
          let withinGuidelineLength = false

          if (emitterDir === "x+") {
            // Guideline extends in +x direction
            withinGuidelineLength =
              deltaXParallel >= 0 && deltaXParallel <= guidelineLength
          } else {
            // emitterDir === "x-", guideline extends in -x direction
            withinGuidelineLength =
              deltaXParallel <= 0 && deltaXParallel >= -guidelineLength
          }
          ...
          const withinActivateDistance =
            Math.abs(deltaXOrthogonal) <= activateDistance
          let withinGuidelineLength = false

          if (emitterDir === "y+") {
            // Guideline extends in +y direction
            withinGuidelineLength =
              deltaYParallel >= 0 && deltaYParallel <= guidelineLength
          } else {
            // emitterDir === "y-", guideline extends in -y direction
            withinGuidelineLength =
              deltaYParallel <= 0 && deltaYParallel >= -guidelineLength
          }
