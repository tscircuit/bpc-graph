...
function calculateEdgeToEdgeDistance(bounds1: Bounds, bounds2: Bounds): number {
  ...
  if (bounds1.maxX < bounds2.minX) { // box1 is entirely to the left of box2
    sep_x = bounds2.minX - bounds1.maxX;
  } else if (bounds2.maxX < bounds1.minX) { // box2 is entirely to the left of box1
    sep_x = bounds1.minX - bounds2.maxX;
  }

  let sep_y = 0;
  // Check for separation along Y-axis
  if (bounds1.maxY < bounds2.minY) { // box1 is entirely below box2
    sep_y = bounds2.minY - bounds1.maxY;
  } else if (bounds2.maxY < bounds1.minY) { // box2 is entirely below box1
    sep_y = bounds1.minY - bounds2.maxY;
  }

  if (sep_x > 0 && sep_y > 0) { // Separated in both axes (diagonal separation)
    return Math.sqrt(sep_x * sep_x + sep_y * sep_y);
  } else if (sep_x > 0) { // Separated only along X-axis (Y projections overlap)
    return sep_x;
  } else if (sep_y > 0) { // Separated only along Y-axis (X projections overlap)
    return sep_y;
  } else { // Boxes are overlapping or touching
    return 0;
  }
}
  ...
  for (let i = 0; i < g.boxes.length; i++) {
    for (let j = i + 1; j < g.boxes.length; j++) {
      ...
      if (edgeDistance <= D1) {
          ...
          forceFactor = 0.0;
      } else { // D1 < edgeDistance < D2 and D1 < D2 (guaranteed ramp conditions)
          forceFactor = (D2 - edgeDistance) / (D2 - D1);
      }

      if (forceFactor <= 0) { // Includes 0 and tiny negatives from float precision
        continue;
      }
