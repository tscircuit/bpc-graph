...
test("GraphNetworkTransformer - debugger page simple case", () => {
  ...
  const hasChangePinColor = transformer.stats.finalOperationChain.some(op => op.operation_type === "change_pin_color");
  const hasChangePinNetwork = transformer.stats.finalOperationChain.some(op => op.operation_type === "change_pin_network");
  const hasMovePin = transformer.stats.finalOperationChain.some(op => op.operation_type === "move_pin");
