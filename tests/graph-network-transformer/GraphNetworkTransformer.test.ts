...
test("GraphNetworkTransformer - empty to single box with pin", () => {
  ...
  const targetGraph: BpcGraph = {
    boxes: [
      { boxId: "B1", kind: "floating", center: { x: 0, y: 0 } },
    ],
    pins: [
  ...
  const hasAddBox = transformer.stats.finalOperationChain.some(op => op.operation_type === "add_box");
  const hasAddPinToBox = transformer.stats.finalOperationChain.some(op => op.operation_type === "add_pin_to_box");
