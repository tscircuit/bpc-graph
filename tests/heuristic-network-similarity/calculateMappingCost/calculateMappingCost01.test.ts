...
test("identical empty graphs", () => {
  const boxAssignment: Assignment<string, string> = { map: new Map(), unmappedRhs: new Set() };
  const networkAssignment: Assignment<string, string> = { map: new Map(), unmappedRhs: new Set() };
...
test("identical single box, single pin graphs", () => {
  const g: BpcGraph = {
    ...
    pins: [{ boxId: "B1", pinId: "P1", offset: { x: 0, y: 0.5 }, color: "red", networkId: "N1" }],
  };
  const boxAssignment: Assignment<string, string> = { map: new Map([["B1", "B1"]]), unmappedRhs: new Set() };
  const networkAssignment: Assignment<string, string> = { map: new Map([["N1", "N1"]]), unmappedRhs: new Set() };
...
test("add one box with one pin", () => {
  ...
  const boxAssignment: Assignment<string, string> = { map: new Map(), unmappedRhs: new Set(["B1"]) };
  // Network N1 in g2 is unmapped from g1's perspective
  const networkAssignment: Assignment<string, string> = { map: new Map(), unmappedRhs: new Set(["N1"]) };
...
test("remove one box with one pin", () => {
  ...
  const boxAssignment: Assignment<string, string> = { map: new Map([["B1", null]]), unmappedRhs: new Set() };
  // Network N1 in g1 maps to null
  const networkAssignment: Assignment<string, string> = { map: new Map([["N1", null]]), unmappedRhs: new Set() };
...
test("change pin color", () => {
  ...
  const g2: BpcGraph = {
    boxes: [{ boxId: "B1", kind: "floating", center: { x: 0, y: 0 } }],
    pins: [{ boxId: "B1", pinId: "P1", offset: { x: 0, y: 0.5 }, color: "blue", networkId: "N1" }],
  };
  const boxAssignment: Assignment<string, string> = { map: new Map([["B1", "B1"]]), unmappedRhs: new Set() };
  const networkAssignment: Assignment<string, string> = { map: new Map([["N1", "N1"]]), unmappedRhs: new Set() };
  const pinDirectionsG1 = precomputePinDirections(g1);
...
test("change pin network", () => {
  ...
  const g2: BpcGraph = {
    boxes: [{ boxId: "B1", kind: "floating", center: { x: 0, y: 0 } }],
    pins: [{ boxId: "B1", pinId: "P1", offset: { x: 0, y: 0.5 }, color: "red", networkId: "N2" }],
  };
  const boxAssignment: Assignment<string, string> = { map: new Map([["B1", "B1"]]), unmappedRhs: new Set() };
  // N1 maps to N2
  const networkAssignment: Assignment<string, string> = { map: new Map([["N1", "N2"]]), unmappedRhs: new Set() };
  const pinDirectionsG1 = precomputePinDirections(g1);
  const pinDirectionsG2 = precomputePinDirections(g2);
...
test("change pin network (N1 unmapped, N2 new)", () => {
  ...
  const networkAssignment: Assignment<string, string> = { map: new Map([["N1", null]]), unmappedRhs: new Set(["N2"]) };
  const pinDirectionsG1 = precomputePinDirections(g1);
  const pinDirectionsG2 = precomputePinDirections(g2);
...
test("add pin to existing mapped box", () => {
  ...
  const g2: BpcGraph = {
    boxes: [{ boxId: "B1", kind: "floating", center: { x: 0, y: 0 } }],
    pins: [
      { boxId: "B1", pinId: "P1", offset: { x: 0, y: 0.5 }, color: "red", networkId: "N1" },
      { boxId: "B1", pinId: "P2", offset: { x: 0, y: -0.5 }, color: "blue", networkId: "N2" },
    ],
  };
  const boxAssignment: Assignment<string, string> = { map: new Map([["B1", "B1"]]), unmappedRhs: new Set() };
...
test("remove pin from existing mapped box", () => {
  ...
  const boxAssignment: Assignment<string, string> = { map: new Map([["B1", "B1"]]), unmappedRhs: new Set() };
  const networkAssignment: Assignment<string, string> = { 
    map: new Map([["N1", "N1"], ["N2", null]]), // N1 maps to N1, N2 is removed
    unmappedRhs: new Set()
  };
  const pinDirectionsG1 = precomputePinDirections(g1);
  const pinDirectionsG2 = precomputePinDirections(g2);
...
test("complex case: add box, remove box, change pin, add pin, remove pin", () => {
  const g1: BpcGraph = {
    ...
    pins: [
      { boxId: "B1", pinId: "P1_B1", offset: { x: 0, y: 0.5 }, color: "red", networkId: "N1" }, // Color change, network change
      { boxId: "B1", pinId: "P2_B1", offset: { x: 0, y: -0.5 }, color: "green", networkId: "N2" }, // To be removed
      { boxId: "B2", pinId: "P1_B2", offset: { x: 0, y: 0.5 }, color: "yellow", networkId: "N3" }, // Part of removed B2
    ],
  };
  const g2: BpcGraph = {
    boxes: [
    ...
    pins: [
      { boxId: "B1_g2", pinId: "P1_B1_g2", offset: { x: 0, y: 0.5 }, color: "blue", networkId: "N1_g2" }, // Mapped from P1_B1
      { boxId: "B1_g2", pinId: "P3_B1_g2", offset: { x: 0.5, y: 0 }, color: "purple", networkId: "N4_g2" }, // New pin in B1_g2
      { boxId: "B3_g2", pinId: "P1_B3_g2", offset: { x: 0, y: 0.5 }, color: "orange", networkId: "N5_g2" }, // Pin in new B3_g2
    ],
  };

  const boxAssignment: Assignment<string, string> = {
    map: new Map([["B1", "B1_g2"], ["B2", null]]),
    unmappedRhs: new Set(["B3_g2"]),
  };
  const networkAssignment: Assignment<string, string> = {
    map: new Map([["N1", "N1_g2"], ["N2", null], ["N3", null]]),
